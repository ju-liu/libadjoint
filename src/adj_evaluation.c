...
int adj_evaluate_functional(adj_adjointer* adjointer, char* functional, adj_scalar* output)
{
  int ierr;
  void (*functional_func)(adj_adjointer* adjointer, int ndepends, adj_variable* variables, adj_vector* dependencies, char* name, adj_scalar* output) = NULL;
  adj_vector* dependencies = NULL;
  int ndepends = 0;
  adj_variable* variables = NULL;
  ...
  /* We have the right callback, so let's call it already */ 
  functional_func(adjointer, ndepends, variables, dependencies, functional, output);
...
int adj_evaluate_functional_derivative(adj_adjointer* adjointer, adj_variable variable, char* functional, adj_vector* output, int* has_output)
{
  ...
  /* Get the dependency list for the functional derivative evaluation. */
  functional_data_ptr = adjointer->functional_data_start;
  while (functional_data_ptr != NULL)
  {
    if (strncmp(functional_data_ptr->name, functional, ADJ_NAME_LEN) == 0)
