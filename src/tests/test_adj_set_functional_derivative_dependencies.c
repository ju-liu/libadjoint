...
#ifndef HAVE_PETSC
void test_adj_set_functional_derivative_dependencies(void)
{
...
void test_adj_set_functional_derivative_dependencies(void)
{
  ...
  /* Try to set the functional derivative dependencies */
  ierr = adj_set_functional_derivative_dependencies(&adjointer, "Drag", u[3], 2, &u[3]);
  adj_test_assert(ierr == ADJ_ERR_INVALID_INPUTS, "Should not have worked fine");

  /* First set the functional dependencies. Claim that the whole of u is necessary for the functional computation. */
  ierr = adj_set_functional_dependencies(&adjointer, "Drag", 5, u);
  adj_test_assert(ierr == ADJ_OK, "Should have worked fine");

  /* Now we can set the functional derivative dependencies */
  ierr = adj_set_functional_derivative_dependencies(&adjointer, "Drag", u[0], 0, NULL);
  adj_test_assert(ierr == ADJ_OK, "Should have worked fine");
  ierr = adj_set_functional_derivative_dependencies(&adjointer, "Drag", u[1], 1, &u[4]);
  adj_test_assert(ierr == ADJ_OK, "Should have worked fine");
  ierr = adj_set_functional_derivative_dependencies(&adjointer, "Drag", u[2], 0, NULL);
  adj_test_assert(ierr == ADJ_OK, "Should have worked fine");
  ...
  adj_test_assert(ierr == ADJ_OK, "Should have worked");
  has_val = has_values(&adjointer, 5, u);
  adj_test_assert(memcmp(has_val, after_2, 5 * sizeof(int)) == 0, "Should be {1, 1, 1, 0, 1}");
  free(has_val);
