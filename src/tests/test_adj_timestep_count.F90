...
subroutine test_adj_timestep_count
  ...
  ierr = adj_timestep_start(adjointer, timestep=2, start=start)
  call adj_test_assert(ierr == ADJ_ERR_INVALID_INPUTS, "No such timestep 2")
  ierr = adj_timestep_end(adjointer, timestep=2, end=end)
  call adj_test_assert(ierr == ADJ_ERR_INVALID_INPUTS, "No such timestep 2")

  ierr = adj_timestep_start(adjointer, timestep=0, start=start)
  call adj_test_assert(start == 0, "Start should be zero")
  ierr = adj_timestep_start(adjointer, timestep=1, start=start)
  call adj_test_assert(start == 2, "Start should be two")

  ierr = adj_timestep_end(adjointer, timestep=0, end=end)
  call adj_test_assert(end == 1, "End should be one")
  ierr = adj_timestep_end(adjointer, timestep=1, end=end)
