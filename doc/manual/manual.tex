\documentclass[11pt,a4paper]{book}
\usepackage{color}
\usepackage{graphicx}
\usepackage{subfigure}
\usepackage{fancyhdr}
\usepackage{subeqnarray}
\usepackage[hyperindex,pagebackref=true]{hyperref}
\usepackage{natbib}
\usepackage{doi}
\usepackage{makeidx}
\usepackage[margin=2cm]{geometry}
\usepackage{float}
\usepackage{xspace}
\usepackage{amsfonts}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsthm}
\usepackage[utf8]{inputenc}
\newcommand{\defapiss}[1]{\subsubsection{\texttt{#1}\label{api:#1}}\index{\texttt{#1}}}
\newcommand{\defapis}[1]{\subsection{\texttt{#1}\label{api:#1}}\index{\texttt{#1}}}
\newcommand{\defapi}[1]{\section{\texttt{#1}\label{api:#1}}\index{\texttt{#1}}}
\newcommand{\refapi}[1]{\hyperref[api:#1]{\texttt{#1}}}
%\newcommand{\defapiss}[1]{\subsubsection{\texttt{#1}}\index{#1}}
%\newcommand{\defapis}[1]{\subsection{\texttt{#1}}\index{#1}}
%\newcommand{\refapi}[1]{\texttt{#1}}
\newcommand{\UnderscoreCommands}{\do\refapi}
\usepackage[strings]{underscore}
\usepackage[tight]{minitoc}

\makeindex
% Command to enable the labelling of the primary reference. 
% To do this use \index{term|primary}
\newcommand{\primary}[1]{\textbf{\hyperpage{#1}}}
\newcommand{\libadjoint}[0]{{\texttt{libadjoint}}\xspace}
\newcommand{\m}[0]{{\vec{m}}}
\newcommand{\adjointer}[0]{\texttt{adj\_adjointer}\xspace}

\newenvironment{block}[1]{%
  \begin{actionenv}%
    \def\insertblocktitle{#1}%
    \par%
    \usebeamertemplate{block begin}}
  {\par%
    \usebeamertemplate{block end}%
  \end{actionenv}}

\definecolor{Grey}{rgb}{0.96,0.97,0.98}
\makeatletter\newenvironment{graybox}{%
   \begin{lrbox}{\@tempboxa}\begin{minipage}{\columnwidth}}{\end{minipage}\end{lrbox}%
   \colorbox{Grey}{\usebox{\@tempboxa}}
}\makeatother

% Cause numbering of subsubsections. 
\setcounter{secnumdepth}{3}
\setcounter{tocdepth}{3}
\setcounter{minitocdepth}{3}

\definecolor{DarkBlue}{rgb}{0.00,0.00,0.55}
\definecolor{Black}{rgb}{0.00,0.00,0.00}
\hypersetup{
    linkcolor   = DarkBlue,
    anchorcolor = DarkBlue,
    citecolor   = DarkBlue,
    filecolor   = DarkBlue,
    pagecolor   = DarkBlue,
    urlcolor    = DarkBlue,
    colorlinks  = true,
    pdftitle    = {\libadjoint manual},
    pdfauthor   = {Patrick Farrell <patrick.farrell06@imperial.ac.uk>}
}

% Define date in european format
\def\today{\number\day\space\ifcase\month\or
 January\or February \or March\or April\or May\or June\or
 July\or August\or September\or October\or November\or December\fi
 \space\number\year}

\floatstyle{boxed}
\newfloat{example}{thp}{leg}[chapter]
\floatname{example}{Example}

\setlength{\parindent}{0.0in} \setlength{\parskip}{0.1in}

\newcommand{\synopsisname}{Abstract}
\newenvironment{synopsis}{%
  \begin{center}%
    {\bfseries \hspace{0.2cm}\synopsisname\vspace{-0.4cm}}%
  \end{center}%
  \begin{quote}
}{%
   \end{quote}
}

\pagestyle{fancy}
\fancyfoot{} % No footers.
\fancyhead[RO,LE]{\thepage}
\fancyhead[RE]{\rightmark}
\fancyhead[LO]{\leftmark}
\renewcommand{\sectionmark}[1]{\renewcommand{\leftmark}{\thesection\qquad #1}}
\renewcommand{\chaptermark}[1]{\renewcommand{\rightmark}{#1}}

\makeatletter
\def\thickhrulefill{\leavevmode \leaders \hrule height 1ex \hfill \kern \z@}
\def\@makechapterhead#1{%
  %\vspace*{50\p@}%
  \vspace*{10\p@}%
  {\parindent \z@ \centering \reset@font
        \thickhrulefill\quad
        \scshape \@chapapp{} \thechapter
        \quad \thickhrulefill
        \par\nobreak
        \vspace*{10\p@}%
        \interlinepenalty\@M
        \hrule
        \vspace*{10\p@}%
        \Huge \bfseries #1\par\nobreak
        \par
        \vspace*{10\p@}%
        \hrule
    %\vskip 40\p@
    \vskip 100\p@
  }}
\def\@makeschapterhead#1{%
  %\vspace*{50\p@}%
  \vspace*{10\p@}%
  {\parindent \z@ \centering \reset@font
        \thickhrulefill
        \par\nobreak
        \vspace*{10\p@}%
        \interlinepenalty\@M
        \hrule
        \vspace*{10\p@}%
        \Huge \bfseries #1\par\nobreak
        \par
        \vspace*{10\p@}%
        \hrule
    %\vskip 40\p@
    \vskip 50\p@
  }}


\begin{document}

\bibliographystyle{plainnat}
\dominitoc[c]

\frontmatter

\begin{titlepage}

\begin{center}

\mbox{}

\vfill

{\bfseries{\huge
\libadjoint manual
\\}}

\vspace{40mm}

{\LARGE
P.E. Farrell and S. W. Funke\\
Applied Modelling \& Computation Group
\\}

\vspace{1mm}


{\LARGE
\url{http://amcg.ese.ic.ac.uk/libadjoint}
}

\vspace{10mm}

{\LARGE
Department of Earth Science and Engineering,\\
Royal School of Mines,\\
Imperial College London,\\
London, SW7 2AZ, UK
\\}

\vspace{80mm}


{\Large
Version 0.8 (revision \input{version}\hspace{-0.14cm}; \today)
\\}

\vfill

\end{center}

\end{titlepage}

\input{overview}

% originals from book.cls
%\newcommand*\l@section{\@dottedtocline{1}{1.5em}{2.3em}}
%\newcommand*\l@subsection{\@dottedtocline{2}{3.8em}{3.2em}}
%\newcommand*\l@subsubsection{\@dottedtocline{3}{7.0em}{4.1em}}
\renewcommand\l@section{\@dottedtocline{1}{1.5em}{2.7em}}
\renewcommand\l@subsection{\@dottedtocline{2}{4.2em}{3.6em}}
\renewcommand\l@subsubsection{\@dottedtocline{3}{7.8em}{4.5em}}
\renewcommand\mtctitle{\hfil\ Contents\hfil}

\makeatother

{ \setlength{\parskip}{0pt} \tableofcontents}

\newpage \clearpage \thispagestyle{plain}

\mainmatter

\include{introduction}
\include{annotation}
\include{callbacks}
\include{types}
\include{assembly}
\include{debugging}

% Whole bunch of format hacking because Bibliography isn't a real chapter.
\cleardoublepage
\phantomsection
\renewcommand\leftmark{}
\renewcommand\rightmark{Bibliography}
\addcontentsline{toc}{chapter}{Bibliography}
\bibliography{literature}

\appendix

\include{error_codes}
\include{todo}

\cleardoublepage
\phantomsection
\renewcommand\leftmark{}
\renewcommand\rightmark{Index}
\addcontentsline{toc}{chapter}{Index}
\printindex

\end{document}
