...
It stores the required details for the assembly of the adjoint equations, in particular the annotation of the forward model and all associated callbacks routines.
...
In particular, the model annotation, the callback registration and the recording functions are replaced by dummy routines within \texttt{libadjoint} that always return \refapi{ADJ_OK}.
...
Adds the equation \texttt{equation} to the \texttt{adjointer}.
...
Creates an \refapi{adj_storage} object that contains the supplied \refapi{adj_vector} \texttt{val}.
...
Therefore, \texttt{val} can be deallocated in the model after the variable has been recorded. 
