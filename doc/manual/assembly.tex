...
This function is used to forget recorded variables. 
It forgets all recorded variables that are not needed to assemble the adjoint equations with index lower than \texttt{equation}.

The data callback \refapi{vec_destroy} must be registered to use this function.
...
However, at the end of the adjoint equations loop, the recorded adjoint variables are no longer needed and can be forgotten using \refapi{adj_forget_adjoint_equation}.
...
Hence the, during the forward run, recorded dependency variables are requested separately for each functional. 
...
Before a full implementation template for the adjoint main loop can be provided, a few helper functions need to be introduced.
...
If the required functional callback, see \refapi{adj_register_functional_callback}, is missing an \refapi{ADJ_ERR_NEED_CALLBACK} is returned.
Furthermore, the evaluation of a functional without any dependencies returns \refapi{ADJ_WARN_UNINITIALISED_VALUE}. 
In both cases, \refapi{output} is uninitialised after the function.
