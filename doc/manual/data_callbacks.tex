...
\defapiss{ADJ_VEC_TO_FILE_CB}
\begin{boxwithtitle}{\texttt{ADJ_VEC_TO_FILE_CB}}
\begin{minipage}{\columnwidth}
\begin{ccode}
  void (*vec_to_file)(adj_vector x, char* filename);
\end{ccode}
\begin{fortrancode}
  subroutine adj_vec_to_file(x, filename) 
    type(adj_vector), intent(in), value :: x
...
\end{minipage}
\end{boxwithtitle}
This data callback saves the provided vector into a file named \texttt{filename} on disk.

When necessary: If a checkpointing scheme is used.


\defapiss{ADJ_VEC_FROM_FILE_CB}
\begin{boxwithtitle}{\texttt{ADJ_VEC_FROM_FILE_CB}}
\begin{minipage}{\columnwidth}
\begin{ccode}
  void (*vec_from_file)(adj_vector* x, char *filename);
\end{ccode}
\begin{fortrancode}
  subroutine adj_vec_from_file(x, filename) 
...
\end{minipage}
\end{boxwithtitle}
This data callback loads the vector from file \texttt{filename} on disk.
